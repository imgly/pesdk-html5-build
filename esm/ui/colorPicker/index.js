...
`;var Q=Re;import N from"react";import ee from"styled-components";const He=ee.div`
  text-align: center;
  width: 28px;
  padding-left: 4px;
`,Pe=ee(g.Input.Text)`
...
`,$e=({value:e,label:r,onChange:t})=>N.createElement(He,null,N.createElement(g.Input.Label,{label:r}),N.createElement(Pe,{min:0,max:255,value:e,onChange:t}));var F=$e;import W from"react";import te from"styled-components";const Me=te.div`
  text-align: left;
  width: 62px;
`,Te=te(g.Input.Text)`
  input {
...
`,Be=({value:e,label:r,onChange:t})=>W.createElement(Me,null,W.createElement(g.Input.Label,{label:r}),W.createElement(Te,{value:e.toUpperCase(),onChange:t}));var re=Be;import O from"react";import je from"styled-components";import Ie from"styled-components";const oe=Ie.div.attrs(e=>({style:{left:e.position.x||`${0}px`,top:e.position.y||`${0}px`}}))`
  &&& {
    position: absolute;
...
`;var H=oe;oe.defaultProps={theme:K};const ne=Ht(Pt());import U from"react";import Fe from"react-dom";import Oe from"styled-components";const De=Oe.div`
  position: relative;
  height: 14px;
  width: 100%;
`;class ie extends U.Component{constructor(e){super(e);this.onDragStart=e=>{this.setState({dragging:!0}),this.onPositionChange(e)};this.onPositionChange=e=>{const{onChange:r,config:t}=this.props,o=t.measurements.color.colorPicker.thumb.size/2,n=t.measurements.color.colorPicker.thumb.size;let i=0,a=0;i=e.clientX,a=e.clientY;const s=Fe.findDOMNode(this);if(s&&s instanceof HTMLElement){const{top:l,left:h,width:d,height:S}=s.getBoundingClientRect();let x=i-h-o,A=a-l-o;x=Math.max(0,Math.min(d-n,x)),A=Math.max(0,Math.min(S-n,A)),r&&r({x,y:A}),this.setState({x,y:A})}};this.onDrag=e=>{this.state.dragging&&this.onPositionChange(e)};this.onDragStop=()=>{this.state.dragging&&this.setState({dragging:!1})};this.state={dragging:!1,x:e.defaultPosition.x,y:e.defaultPosition.y},this.debouncedOnDrag=ne.default(this.onDrag,10)}componentWillUnmount(){this.setState({dragging:!1})}render(){const{surface:e,setInnerRef:r,className:t,children:o}=this.props;return U.createElement(De,{ref:r,className:t},U.createElement(g.DraggableCore,{onStart:this.onDragStart,onDrag:this.debouncedOnDrag,onStop:this.onDragStop},e),o)}}ie.defaultProps={defaultPosition:{x:0,y:0}};var P=w("config","active")(y(ie));import{css as Ge}from"styled-components";const Ee=Ge`
  position: absolute;
  top: 3px;
  left: 0;
  width: 100%;
...
`;var k=Ee;const Le=je.div`
  ${k}
  background: linear-gradient(90deg, #F00 0%, #FF0 16.66%, #0F0 33.33%, #0FF 50%, #00F 66.66%, #F0F 83.33%, #F00 100%);
  height: 8px;
`;class Ve extends O.Component{constructor(e){super(e);this.onKnob=({x:e})=>{const{onChange:r,config:t}=this.props,{offsetWidth:o}=this.node,n=o-t.measurements.color.colorPicker.thumb.size,i=e*o/n,a=Math.max(0,Math.min(360,i/o*360));r(a),this.setState({x:e})};this.setPositionFromColor=e=>{const{config:r}=this.props,{offsetWidth:t}=this.node;let o=0;const n=t-r.measurements.color.colorPicker.thumb.size;this.node&&(o=e.toHsv().h*t/360),this.setState({x:o*n/t})};this.nodeRef=r=>{this.node=r},this.state={x:0}}render(){const{x:e}=this.state;return O.createElement(P,{surface:O.createElement(Le,null),setInnerRef:this.nodeRef,onChange:this.onKnob},O.createElement(H,{position:{x:e,y:0}}))}}var se=w("config")(y(Ve));import T from"react";import ae from"styled-components";const ze=ae.div.attrs(e=>({style:{background:`linear-gradient(to left, ${e.transparentColor} 0%, ${e.opaqueColor} 100%)`}}))`
  ${k}
  z-index: 1;
`,qe=ae.div.attrs(e=>({style:{backgroundImage:`url(${e.url})`}}))`
  ${k}
  z-index: 0;
  background-repeat: repeat-x;
  background-size: contain;
  background-position: left;
`;class Ne extends T.Component{constructor(e){super(e);this.onKnob=({x:e})=>{const{onChange:r,config:t}=this.props,{offsetWidth:o}=this.node,n=o-t.measurements.color.colorPicker.thumb.size,i=e*o/n,a=(o-i)/o,s=Math.round((a+Number.EPSILON)*100)/100,l=Math.min(1,Math.max(0,s));r(l),this.setState({x:e})};this.setPositionFromColor=e=>{const{config:r}=this.props,{offsetWidth:t}=this.node;let o=0;const n=t-r.measurements.color.colorPicker.thumb.size;this.node&&(o=(1-e.toHsv().a)*t),this.setState({x:o*n/t})};this.nodeRef=r=>{this.node=r},this.state={x:0}}render(){const{opaqueColor:e,transparentColor:r,config:t}=this.props,{x:o}=this.state,n=t.assetProvider.getPath("controls","/tiled-background.png");return T.createElement(P,{surface:T.createElement(ze,{opaqueColor:e,transparentColor:r}),setInnerRef:this.nodeRef,onChange:this.onKnob},T.createElement(qe,{url:n}),T.createElement(H,{position:{x:o,y:0}}))}}var le=w("config")(y(Ne));import D from"react";import he from"styled-components";const We=he.div.attrs(e=>({style:{backgroundColor:`hsl(${e.hue}, 100%, 50%)`}}))`
  ${k}
